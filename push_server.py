...
def send_fcm(title, key):
    ...
    message_title = title
    message_body = "달리자대피소를 켜서 가까운 대피소로 달리세요!"
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for emergency in current:
            print(emergency)
            if int(emergency['tmSeq']) > int(pre[0]['tmSeq']) and '발표' in emergency['title']:
                send_fcm(emergency['title'].split(' / ')[1].split('(')[0], firebase_api_key)
            else:
                break
